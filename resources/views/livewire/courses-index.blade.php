    ...
    <div class="bg-gray-200 py-4 mb-16">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex">
            <button class="bg-white shadow h-12 px-4 rounded-lg text-gray-700 mr-4" wire:click="resetFilters">
                <i class="fas fa-align-justify text-xs mr-2"></i>
                Todos los cursos
            </button>
                <!-- Dropdown Categoria -->
            ...
            <div class="relative mr-4" x-data="{ open:false }">
                ...
                <div class="absolute right-0 w-40 mt-2 py-2 bg-white border rounded shadow-xl" x-show="open" x-on:click.away="open = false">   
                    @foreach ($subcategories as $subcategory)
                            <a class="cursor-pointer transition-colors duration-200 block px-4 py-2 text-normal text-gray-900 rounded hover:bg-blue-500 hover:text-white" wire:click="$set('subcategory_id', {{$subcategory->id}})" x-on:click="open = false">{{$subcategory->name}}</a>
                    @endforeach
                </div>
                <!-- // Dropdown Body -->
            </div>
